...
// ── Support tickets ──
export async function sendSupportTicket({ name, email, reason, message, account }){
  if(!process.env.RESEND_API_KEY) throw new Error("RESEND_API_KEY is not set");
  ...
  const html = `<div style="font-family:Arial,Helvetica,sans-serif;max-width:580px;margin:0 auto;">
    ...
    ${account ? `<div style="margin-top:16px;border-top:1px solid #e0d9c8;padding-top:12px;"><div style="font-size:13px;color:#8a6d1f;font-weight:bold;">Account found (${esc(account.source)})</div><table style="font-size:13px;color:#333;line-height:1.6;margin-top:4px;"><tr><td style="padding:1px 12px 1px 0;"><strong>Name</strong></td><td>${esc((account.profile&&account.profile.name)||"—")}</td></tr><tr><td style="padding:1px 12px 1px 0;"><strong>Account email</strong></td><td>${esc(account.email)}</td></tr><tr><td style="padding:1px 12px 1px 0;"><strong>User ID</strong></td><td>${esc(account.id)}</td></tr><tr><td style="padding:1px 12px 1px 0;"><strong>Premium</strong></td><td>${account.profile&&account.profile.is_premium?"Yes":"No"}${account.profile&&account.profile.subscription_status?` (${esc(account.profile.subscription_status)})`:""}</td></tr><tr><td style="padding:1px 12px 1px 0;"><strong>Admin</strong></td><td>${account.profile&&account.profile.is_admin?"Yes":"No"}</td></tr><tr><td style="padding:1px 12px 1px 0;"><strong>Email verified</strong></td><td>${account.email_confirmed?"Yes":"No"}</td></tr><tr><td style="padding:1px 12px 1px 0;"><strong>Created</strong></td><td>${esc(account.created_at)}</td></tr><tr><td style="padding:1px 12px 1px 0;"><strong>Last sign-in</strong></td><td>${esc(account.last_sign_in_at||"—")}</td></tr></table>${(()=>{const d=(account.profile&&account.profile.data)||{};return `<div style="margin-top:6px;font-size:12px;color:#555;">Role: ${esc(d.role||"—")} · Experience: ${esc(d.experience||"—")} · Location: ${esc(d.location||"—")}</div>`;})()}</div>` : `<div style="margin-top:16px;border-top:1px solid #e0d9c8;padding-top:12px;font-size:12px;color:#777;">No matching account found (not logged in, and the email doesn’t match a registered user).</div>`}
  </div>`;
  return resend.emails.send({ from: FROM, to, subject: `[Support · ${reason}] from ${name}`, html });
}

// ── Password changed confirmation ──
export function passwordChangedEmailHtml(name){
  return shell(`
    <p style="font-size:15px;color:#d8ceb4;line-height:1.6;margin:0 0 14px;">${name ? "Hi " + escapeHtml(name) + "," : "Hi,"}</p>
    <p style="font-size:14px;color:rgba(216,206,180,.85);line-height:1.7;margin:0 0 14px;">The password for your Main Quest account was just changed.</p>
    <p style="font-size:14px;color:rgba(216,206,180,.85);line-height:1.7;margin:0 0 18px;">If this was you, no action is needed. If you did <strong style="color:#e8a06a;">not</strong> make this change, reset your password immediately and contact support.</p>
    <div style="text-align:center;margin:0 0 8px;"><a href="${SITE_URL}/" style="display:inline-block;font-size:13px;font-weight:bold;color:#0a0608;background:#c9a84c;text-decoration:none;padding:12px 26px;border-radius:10px;font-family:Georgia,serif;">Go to Main Quest</a></div>
  `);
}
export async function sendPasswordChangedEmail(to, name){
  if(!process.env.RESEND_API_KEY) return;
  const r = await resend.emails.send({ from: FROM, to, subject: "Your Main Quest password was changed", html: passwordChangedEmailHtml(name) });
  if(r && r.error){ const m = typeof r.error==="string"?r.error:(r.error.message||JSON.stringify(r.error)); throw new Error(m); }
  return r;
}

// ── New-device sign-in alert ──
export function newDeviceEmailHtml(name, info){
  const when = new Date().toUTCString();
  return shell(`
    <p style="font-size:15px;color:#d8ceb4;line-height:1.6;margin:0 0 14px;">${name ? "Hi " + escapeHtml(name) + "," : "Hi,"}</p>
    <p style="font-size:14px;color:rgba(216,206,180,.85);line-height:1.7;margin:0 0 16px;">We noticed a sign-in to your Main Quest account from a new device or browser.</p>
    <table role="presentation" width="100%" style="margin:0 0 18px;border:1px solid #2a2018;border-radius:10px;background:#120c10;border-collapse:separate;">
      <tr><td style="padding:11px 16px;font-size:13px;color:#b8ad90;">When</td><td style="padding:11px 16px;font-size:13px;color:#d8ceb4;text-align:right;">${escapeHtml(when)}</td></tr>
      <tr><td style="padding:11px 16px;font-size:13px;color:#b8ad90;border-top:1px solid #2a2018;">Device</td><td style="padding:11px 16px;font-size:13px;color:#d8ceb4;text-align:right;border-top:1px solid #2a2018;">${escapeHtml((info&&info.device)||"Unknown")}</td></tr>
    </table>
    <p style="font-size:14px;color:rgba(216,206,180,.85);line-height:1.7;margin:0 0 8px;">If this was you, you can ignore this email. If not, change your password right away.</p>
  `);
}
export async function sendNewDeviceEmail(to, name, info){
  if(!process.env.RESEND_API_KEY) return;
  const r = await resend.emails.send({ from: FROM, to, subject: "New sign-in to your Main Quest account", html: newDeviceEmailHtml(name, info) });
  if(r && r.error){ const m = typeof r.error==="string"?r.error:(r.error.message||JSON.stringify(r.error)); throw new Error(m); }
  return r;
}

// ── Weekly digest (opt-in; sent by the weekly-digest cron) ──
export function weeklyDigestHtml(name, jobs, stats){
  const rows = jobs.map(j => `
    <tr><td style="padding:14px 16px;border:1px solid #2a2018;border-radius:10px;background:#120c10;">
      <div style="font-family:Georgia,serif;font-size:16px;color:#f0d080;font-weight:bold;">${escapeHtml(j.title)}</div>
      <div style="font-size:13px;color:#b8ad90;margin-top:4px;">${escapeHtml(j.company)}${j.location ? " &middot; " + escapeHtml(j.location) : ""}</div>
      ${j.url ? `<a href="${escapeHtml(j.url)}" style="display:inline-block;margin-top:11px;font-size:12px;color:#0a0608;background:#c9a84c;text-decoration:none;padding:7px 16px;border-radius:8px;font-weight:bold;">View posting &rarr;</a>` : ""}
    </td></tr>
    <tr><td style="height:10px;line-height:10px;">&nbsp;</td></tr>`).join("");
  return shell(`
    <p style="font-size:15px;color:#f0d080;font-family:Georgia,serif;margin:0 0 6px;">${name?"Hail, "+escapeHtml(name)+".":"Hail, adventurer."}</p>
    <p style="font-size:14px;color:rgba(216,206,180,.85);line-height:1.6;margin:0 0 4px;">Here are new roles from the past week matching your alerts and followed studios:</p>
    ${stats ? `<table role="presentation" width="100%" style="margin:14px 0 4px;border-collapse:separate;border-spacing:8px 0;"><tr><td style="width:33%;text-align:center;background:#120c10;border:1px solid #2a2018;border-radius:10px;padding:12px 6px;"><div style="font-family:Georgia,serif;font-size:22px;color:#f0d080;font-weight:bold;">${stats.applied||0}</div><div style="font-size:10px;color:#8a7f68;text-transform:uppercase;letter-spacing:.5px;margin-top:3px;">Applied this week</div></td><td style="width:33%;text-align:center;background:#120c10;border:1px solid #2a2018;border-radius:10px;padding:12px 6px;"><div style="font-family:Georgia,serif;font-size:22px;color:#f0d080;font-weight:bold;">${stats.saved||0}</div><div style="font-size:10px;color:#8a7f68;text-transform:uppercase;letter-spacing:.5px;margin-top:3px;">Saved this week</div></td><td style="width:33%;text-align:center;background:#120c10;border:1px solid #2a2018;border-radius:10px;padding:12px 6px;"><div style="font-family:Georgia,serif;font-size:22px;color:#f0d080;font-weight:bold;">${stats.aiUses||0}</div><div style="font-size:10px;color:#8a7f68;text-transform:uppercase;letter-spacing:.5px;margin-top:3px;">AI uses this month</div></td></tr></table>` : ""}
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:16px;">${rows}</table>
    <div style="text-align:center;margin-top:18px;"><a href="${SITE_URL}/" style="font-size:13px;color:#c9a84c;text-decoration:none;">Open the full job board &rarr;</a></div>
    <p style="font-size:11px;color:#5f5749;text-align:center;margin-top:28px;line-height:1.5;">You’re getting this weekly digest because you turned it on.<br/>Manage it anytime in Account &rarr; Notifications.</p>
  `);
}
export async function sendWeeklyDigest(to, name, jobs, stats){
  if(!process.env.RESEND_API_KEY) return;
  const count = jobs.length;
  const r = await resend.emails.send({ from: FROM, to, subject: `Your weekly Main Quest digest — ${count} new ${count===1?"role":"roles"}`, html: weeklyDigestHtml(name, jobs, stats) });
  if(r && r.error){ const m = typeof r.error==="string"?r.error:(r.error.message||JSON.stringify(r.error)); throw new Error(m); }
  return r;
}