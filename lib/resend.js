...
const FROM = process.env.RESEND_FROM || "Main Quest <alerts@mainquestjobs.com>";
...
export function weeklyDigestHtml(name, jobs){
  const rows = jobs.map(j => `
    <tr><td style="padding:14px 16px;border:1px solid #2a2018;border-radius:10px;background:#120c10;">
...
export async function sendWeeklyDigest(to, name, jobs){
  if(!process.env.RESEND_API_KEY) return;
  const count = jobs.length;
  const r = await resend.emails.send({ from: FROM, to, subject: `Your weekly Main Quest digest — ${count} new ${count===1?"role":"roles"}`, html: weeklyDigestHtml(name, jobs) });
  if(r && r.error){ const m = typeof r.error==="string"?r.error:(r.error.message||JSON.stringify(r.error)); throw new Error(m); }
  return r;
}
