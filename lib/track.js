...
// Whether the current visitor is signed in — set by the app on login/logout so
// every event records auth state (lets the dashboard split signed-in vs guest).
let _authed = false;
...
export function track(type, data = {}) {
  ...
  try {
    fetch("/api/track", {
      ...
      keepalive: true,
      body: JSON.stringify({ type, visitor, authed: _authed, ...data }),
    }).catch(() => {});
  } catch (e) {}
  try {
