...
const EMAIL_SPECIFICS = {
  ...
  "Innovated Studios": "Resume and brief introduction",
...
const EMAIL_APPLY_JOBS = {
  ...
  "FarSight Studio": { jobs:[
    ...
    { title:"3D Artist", applyEmail:"artistjobs@farsightstudios.com", experience:"Mid Level", type:"Full-time", isRemote:false, summary:"We are also always on the lookout for well-rounded 3d artists at both junior and senior levels. You must be able to model and texture using 3DS Max and Maya. Traditional drawing skills are also highly valued. Skill at creating low to medium polygon models with efficient texture maps is vital. Applications must include a resume and portfolio (links and attachments are fine) to be considered. Email your resume to artistjobs@farsightstudios.com.", responsibilities:[], requirements:["Bachelor's degree or equivalent","Self motivated, solid communicator","High quality deadline driven work","Able to give and receive constructive criticism","3DS Max, Maya"] },
  ]},
};
...
// Alert-wizard role list — derived from TITLE_CATEGORIES so it always matches the profile & onboarding.
const JOB_CATS = Object.values(TITLE_CATEGORIES).flat();
...
// ATS_STUDIOS now lives in lib/studios.js (imported at top).
...
const COUNTRY_ALIAS = {
  "p.r. china":"China","pr china":"China","p.r china":"China",
...
const CITY_COUNTRY = {
  "helsinki metropolitan area":"Finland","raanana":"Israel","ra'anana":"Israel","tblisi":"Georgia (country)","guoco midtown":"Singapore","gent":"Belgium","jogja":"Indonesia","yogyakarta":"Indonesia","idf":"France","brittany":"France","bretagne":"France","uxbridge":"United Kingdom","greater london":"United Kingdom","city of london":"United Kingdom","horsham":"United Kingdom","farnham":"United Kingdom",
...
const DOMESTIC_CITY_STATE = {
  "san rafael":"CA","sanfransico":"CA","sanfransico bay area":"CA","san francisco bay area":"CA","los angeles office":"CA","troy":"NY",
...
function cityLabelsForState(raw, stateName){
  if(!raw) return [];
  // Exact-override location with a named city (e.g. postcode "RH12" → Horsham).
  const ex=EXACT_PLACE[normLoc(raw)];
  if(ex&&typeof ex==="object"&&ex.city&&ex.sub===stateName) return [`${ex.city}, ${stateName}`];
  ...
  if(!abbr){
    // Not a state/province and not a mapped country → an "Other Locations" bucket.
    // Show the raw location text, tidied, so the listing still reads sensibly.
    const chunks=raw.split(/;|\/|\||\u2022/).map(s=>s.trim()).filter(Boolean);
    const out=[...new Set(chunks.map(c=>titleCasePlace(c)).filter(Boolean))];
    return out.length?out:[stateName];
  }
...
// Exact whole-string location overrides. Checked FIRST in jobPlace, so these win
// over the general matching. Value is either a sentinel ("REMOTE") or {top,sub}
// (with an optional `city` used only for the sub-tab label). Keys are the location
// text, lowercased with collapsed whitespace.
const EXACT_PLACE = {
  "any":"REMOTE","global":"REMOTE","other":"REMOTE","multiple locations":"REMOTE",
  "wa or ca":"REMOTE","bad robot games":"REMOTE",
  "us":{top:"Europe",sub:"Poland"},                       // per location doc — a job labelled just "US" that is actually in Poland
  "santa monica or san rafael":{top:"United States",sub:"California",city:"Santa Monica"},
  "montreal or toronto":{top:"Canada",sub:"Quebec",city:"Montreal"},
  "hong kong; shenzhen":{top:"Asia",sub:"Hong Kong",city:"Hong Kong"},
  "hangzhou office; shanghai office":{top:"Asia",sub:"China",city:"Shanghai"},
  "e15 2gw":{top:"Europe",sub:"United Kingdom",city:"London"},
  "rh12":{top:"Europe",sub:"United Kingdom",city:"Horsham"},
  "gu9 7pt":{top:"Europe",sub:"United Kingdom",city:"Farnham"},
};
const normLoc=(s)=>String(s==null?"":s).toLowerCase().replace(/\s+/g," ").trim();
...
const titleCasePlace=(s)=>s.replace(/\s+/g," ").trim().replace(/\b[\p{L}][\p{L}'’-]*/gu,w=>w[0].toUpperCase()+w.slice(1).toLowerCase());
// Derive a stable sub-region label for an unmapped location string.
function otherRegionName(raw,parts){
  const pick=(parts&&parts.length>1)?parts[parts.length-1]:raw;
  const name=titleCasePlace(String(pick||"").replace(/[()]/g,""));
  if(!name||!/\p{L}/u.test(name)||name.length>40) return null;
  return name;
}
...
// normalizeATSJob is imported from ../lib/normalize (single source of truth)
...
const WORLD_HOTSPOTS = [
  ...
  [36.75,3.06,   0.17], // Algiers, Algeria
  [36.80,10.18,  0.17], // Tunis, Tunisia
  [33.57,-7.59,  0.19], // Casablanca, Morocco
  [31.20,29.92,  0.15], // Alexandria, Egypt
  [-1.94,30.06,  0.16], // Kigali, Rwanda
  [3.85,11.50,   0.15], // Yaoundé, Cameroon
  [4.05,9.70,    0.14], // Douala, Cameroon
  [-20.16,57.50, 0.15], // Port Louis, Mauritius
  [5.56,-0.20,   0.14], // Accra, Ghana
  // ── East Asia ──
  ...
  [25.67,-100.31,0.21], // Monterrey, Mexico
  [9.93,-84.08,  0.17], // San José, Costa Rica
  [13.69,-89.19, 0.15], // San Salvador, El Salvador
  [-12.05,-77.04,0.23], // Lima, Peru
  [-0.18,-78.47, 0.18], // Quito, Ecuador
  [-25.28,-57.63,0.16], // Asunción, Paraguay
  [-25.43,-49.27,0.19], // Curitiba, Brazil
  [-27.59,-48.55,0.17], // Florianópolis, Brazil
  [-19.92,-43.94,0.17], // Belo Horizonte, Brazil
  [-8.05,-34.88, 0.16], // Recife, Brazil
  [6.24,-75.58,  0.19], // Medellín, Colombia
  [-31.42,-64.18,0.15], // Córdoba, Argentina
];
...
// ── JOB ALERTS: model + matching ──────────────────────────────────────────────
// jobAlerts is an array of blocks: {id, roles[], seniority[], locations[], companies[], matchAll}.
// Older single-object configs are migrated on read so nothing breaks.
function asAlertArray(ja){
...
function jobMatchesAnyAlert(job,alerts){ return asAlertArray(alerts).some(a=>jobMatchesOneAlert(job,a)); }

// Autocomplete input that turns picks into removable chips (for location & company).
function AutoInput({options,selected,onAdd,onRemove,placeholder}){
  const [q,setQ]=useState("");
  const ql=q.trim().toLowerCase();
  const matches=ql?options.filter(o=>o.toLowerCase().includes(ql)&&!selected.includes(o)).slice(0,8):[];
  const inp={background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",color:"#f4edd8",colorScheme:"dark",borderRadius:8,padding:"10px 12px",fontSize:12,fontFamily:"inherit",width:"100%",boxSizing:"border-box",outline:"none"};
  const add=(v)=>{const t=String(v).trim();if(t&&!selected.includes(t))onAdd(t);setQ("");};
  return <div style={{position:"relative"}}>
    <input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();add(q);}}} placeholder={placeholder} style={inp}/>
    {matches.length>0&&<div style={{position:"absolute",left:0,right:0,top:"100%",marginTop:4,background:"#140d14",border:"1px solid rgba(201,168,76,.3)",borderRadius:8,zIndex:5,maxHeight:180,overflowY:"auto",boxShadow:"0 12px 34px rgba(0,0,0,.5)"}}>
      {matches.map(m=><div key={m} onClick={()=>add(m)} style={{padding:"8px 11px",fontSize:12,color:"rgba(244,237,216,.8)",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(201,168,76,.1)"} onMouseLeave={e=>e.currentTarget.style.background=""}>{m}</div>)}
    </div>}
    {selected.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:8}}>
      {selected.map(s=><span key={s} style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(201,168,76,.12)",border:"1px solid rgba(201,168,76,.3)",color:"#f0d080",borderRadius:20,fontSize:11,padding:"3px 6px 3px 10px"}}>{s}<span onClick={()=>onRemove(s)} style={{cursor:"pointer",fontSize:14,lineHeight:1,color:"rgba(244,237,216,.6)"}}>×</span></span>)}
    </div>}
  </div>;
}
...
function JobAlertManager({alerts,onSave,isPremium,isAdmin,companyOptions,locationOptions}){
  ...
  const summarize=(a)=>{const parts=[];if(a.roles&&a.roles.length)parts.push("Roles: "+a.roles.join(", "));if(a.seniority&&a.seniority.length)parts.push("Seniority: "+a.seniority.join(", "));if(a.locations&&a.locations.length)parts.push("Locations: "+a.locations.join(", "));if(a.companies&&a.companies.length)parts.push("Companies: "+a.companies.join(", "));return parts;};
  const lbl={fontSize:10,color:"rgba(201,168,76,.6)",textTransform:"uppercase",letterSpacing:.6,marginBottom:8,fontFamily:"'Cinzel',serif"};
  const content = wiz ? (()=>{ const d=wiz.draft; const empty=summarize(d).length===0;
    return <div style={{background:"rgba(201,168,76,.04)",border:"1px solid rgba(201,168,76,.2)",borderRadius:12,padding:14}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <div style={{fontFamily:"'Cinzel',serif",fontSize:13,fontWeight:700,color:"#f0d080"}}>{wiz.editId?"Edit alert":"New alert"} — {STEPS[wiz.step]}</div>
        <div style={{fontSize:10,color:"rgba(244,237,216,.4)"}}>Step {wiz.step+1} of {STEPS.length}</div>
      </div>
      {wiz.step===0&&<div><div style={lbl}>Which roles?</div><div style={{maxHeight:220,overflowY:"auto",border:"1px solid rgba(201,168,76,.1)",borderRadius:8,padding:"6px 8px"}}><CheckGroup opts={JOB_CATS} sel={d.roles} onChange={v=>setD({roles:v})}/></div></div>}
      {wiz.step===1&&<div><div style={lbl}>Which seniority levels?</div><CheckGroup opts={senOpts} sel={d.seniority} onChange={v=>setD({seniority:v})}/></div>}
      {wiz.step===2&&<div><div style={lbl}>Which locations?</div><AutoInput options={locationOptions} selected={d.locations} onAdd={v=>setD({locations:[...d.locations,v]})} onRemove={v=>setD({locations:d.locations.filter(x=>x!==v)})} placeholder="Type a city, state, country or 'Remote'…"/></div>}
      {wiz.step===3&&<div><div style={lbl}>Which companies?</div><AutoInput options={companyOptions} selected={d.companies} onAdd={v=>setD({companies:[...d.companies,v]})} onRemove={v=>setD({companies:d.companies.filter(x=>x!==v)})} placeholder="Type a studio name…"/></div>}
      {wiz.step===4&&<div>
        <div style={lbl}>Review</div>
        <div style={{fontSize:12,color:"rgba(244,237,216,.7)",lineHeight:1.6,marginBottom:12}}>{empty?<span style={{color:"rgba(244,237,216,.4)",fontStyle:"italic"}}>No criteria picked — go back and choose at least one.</span>:summarize(d).map((s,i)=><div key={i}>• {s}</div>)}</div>
        <div onClick={()=>setD({matchAll:!d.matchAll})} style={{display:"flex",alignItems:"flex-start",gap:9,cursor:"pointer",padding:"10px 12px",background:"rgba(201,168,76,.04)",border:"1px solid rgba(201,168,76,.12)",borderRadius:9}}>
          <div style={{width:16,height:16,borderRadius:4,border:`1.5px solid ${d.matchAll?"#c9a84c":"rgba(201,168,76,.25)"}`,background:d.matchAll?"#c9a84c":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>{d.matchAll&&<I.Check s={10} c="#0a0608"/>}</div>
          <div><div style={{fontSize:12,fontWeight:600,color:"#f4edd8"}}>Match all criteria</div><div style={{fontSize:10.5,color:"rgba(244,237,216,.4)",marginTop:2,lineHeight:1.4}}>On = only alert when every criterion is true. Off = alert if any match.</div></div>
        </div>
      </div>}
      <div style={{display:"flex",justifyContent:"space-between",gap:8,marginTop:14}}>
        ...
        {wiz.step<4?
          ...
          :<button onClick={commit} disabled={empty} style={{background:empty?"rgba(201,168,76,.2)":"linear-gradient(135deg,#c9a84c,#f0d080)",border:"none",color:"#0a0608",cursor:empty?"default":"pointer",borderRadius:8,padding:"9px 20px",fontSize:11,fontFamily:"'Cinzel',serif",fontWeight:800,letterSpacing:.5,opacity:empty?.5:1}}>Save alert</button>}
      </div>
    </div>;
  })() : (
    <div>
      {alerts.length>0&&<div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:10}}>
        {alerts.map(a=><div key={a.id} style={{background:"rgba(201,168,76,.04)",border:"1px solid rgba(201,168,76,.15)",borderRadius:10,padding:"11px 12px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
            <div style={{flex:1,minWidth:0}}>{summarize(a).map((s,i)=><div key={i} style={{fontSize:11.5,color:"rgba(244,237,216,.7)",lineHeight:1.5}}>{s}</div>)}</div>
            <div style={{display:"flex",gap:6,flexShrink:0}}>
              <button onClick={()=>edit(a)} style={{background:"rgba(201,168,76,.1)",border:"1px solid rgba(201,168,76,.25)",color:"#f0d080",cursor:"pointer",borderRadius:7,padding:"4px 9px",fontSize:10,fontFamily:"'Cinzel',serif",fontWeight:600}}>Edit</button>
              <button onClick={()=>onSave(alerts.filter(x=>x.id!==a.id))} title="Delete" style={{background:"rgba(232,97,58,.1)",border:"1px solid rgba(232,97,58,.3)",color:"#e8a070",cursor:"pointer",borderRadius:7,width:26,fontSize:13,lineHeight:1}}>×</button>
            </div>
          </div>
          <div onClick={()=>onSave(alerts.map(x=>x.id===a.id?{...x,matchAll:!x.matchAll}:x))} style={{display:"flex",alignItems:"center",gap:7,marginTop:9,cursor:"pointer"}}>
            <div style={{width:15,height:15,borderRadius:4,border:`1.5px solid ${a.matchAll?"#c9a84c":"rgba(201,168,76,.25)"}`,background:a.matchAll?"#c9a84c":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{a.matchAll&&<I.Check s={9} c="#0a0608"/>}</div>
            <span style={{fontSize:10.5,color:"rgba(244,237,216,.5)"}}>Must match all criteria</span>
          </div>
        </div>)}
      </div>}
      {atAlertLimit
...
const JobCard = memo(function JobCard({job,user,guest,onRequestLogin,onApplied,onShowBreakdown,isPlus,isPremium,onToggleSave,activeBreakdown,flatView,notifyOn,onToggleNotify}) {
  ...
  return <div style={{perspective:"1400px"}}><div style={{position:"relative",transformStyle:"preserve-3d",transition:"transform .55s ease",transform:flipped?"rotateY(180deg)":"none"}}><div style={{backfaceVisibility:"hidden",WebkitBackfaceVisibility:"hidden",position:flipped?"absolute":"relative",top:0,left:0,width:"100%"}}><div style={{background:"rgba(16,10,22,.6)",border:`1px solid ${isApplied?"rgba(126,207,179,.3)":job.isNew?"rgba(192,50,26,.35)":"rgba(201,168,76,.12)"}`,borderRadius:10,padding:"13px 15px",transition:"all .2s",cursor:"default",position:"relative",boxShadow:activeBreakdown?"0 0 0 2px rgba(201,168,76,.6), 0 6px 26px rgba(201,168,76,.14)":"none"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(201,168,76,.05)";e.currentTarget.style.transform="translateX(3px)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(16,10,22,.6)";e.currentTarget.style.transform="";}}>
    ...
    {job.company&&<div style={{display:"flex",alignItems:"center",gap:9,marginBottom:4,flexWrap:"wrap"}}>
      ...
      <span style={{position:"relative",marginLeft:"auto",flexShrink:0,display:"flex",alignItems:"center",gap:6}}>{job.isNew&&<I.Alert s={16}/>}
        ...
        <span style={{position:"relative",display:"flex"}}>
          <button onClick={e=>{e.stopPropagation(); if(mobile&&typeof navigator!=="undefined"&&navigator.share){ const _u=(typeof window!=="undefined"?window.location.origin:"")+"/j/"+encodeJob(job); const _t=`${job.title} at ${job.company}${job.location?" · "+job.location:""} — via Main Quest`; track("job_share",{jobKey:`${job.company}|${job.title}|${job.location||""}`,company:job.company,meta:{via:"native"}}); try{navigator.share({title:job.title+" at "+job.company,text:_t,url:_u});}catch(err){} } else { setShowShare(v=>!v); } }} title="Share this job" style={{display:"flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:7,background:showShare?"rgba(201,168,76,.15)":"rgba(201,168,76,.06)",border:`1px solid ${showShare?"rgba(201,168,76,.4)":"rgba(201,168,76,.18)"}`,color:"#c9a84c",cursor:"pointer"}}>
    ...
    {/* Action buttons */}
    <div style={{display:"flex",gap:8,flexWrap:mobile?"nowrap":"wrap",alignItems:"center"}}>
      <button onClick={onApply} style={{background:G,border:"none",color:"#0a0608",cursor:"pointer",borderRadius:7,padding:mobile?"9px 16px":"8px 18px",fontSize:11,fontWeight:800,fontFamily:"'Cinzel',serif",letterSpacing:.5,display:"inline-flex",alignItems:"center",gap:6,flex:"none",justifyContent:"center"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-1px)";e.currentTarget.style.boxShadow="0 4px 16px rgba(201,168,76,.35)";}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}}>{job.isEmailApply?<><I.Send s={12} c="#0a0608"/>Apply by Email</>:<><I.Arrow s={12} c="#0a0608"/>View &amp; Apply</>}</button>
      {(cContact||cEmail)&&<div style={mobile?{position:"absolute",bottom:12,right:12,zIndex:5,display:"flex",gap:6,flexWrap:"wrap"}:{position:"absolute",bottom:13,right:15,zIndex:4,display:"flex",gap:6,flexWrap:"wrap",justifyContent:"flex-end"}}>
    ...
    {/* Match score square */}
    {guest&&!_isOpenAppJob(job)&&<div onClick={e=>{e.stopPropagation();onRequestLogin&&onRequestLogin();}} title="Sign up or log in to access this feature" style={{flexShrink:0,alignSelf:mobile?"flex-start":"stretch",width:mobile?58:66,position:"relative",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,background:"rgba(201,168,76,.05)",border:"1px solid rgba(201,168,76,.18)",borderRadius:10,padding:"6px 4px",alignSelf:"flex-start",cursor:"pointer",overflow:"hidden"}} onMouseEnter={e=>{const t=e.currentTarget.querySelector(".gtip");if(t)t.style.opacity=1;}} onMouseLeave={e=>{const t=e.currentTarget.querySelector(".gtip");if(t)t.style.opacity=0;}}>
    ...
    {user&&!_isOpenAppJob(job)&&<div onClick={e=>{e.stopPropagation(); if(!match){setShowScoreInfo(v=>!v);} else if(smallBreak){setFlipped(true);} else if(onShowBreakdown){onShowBreakdown(job);}}} title="Click for the full match breakdown" style={{flexShrink:0,alignSelf:mobile?"flex-start":"stretch",width:mobile?58:66,position:"relative",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,background:match?`${scoreColor}1a`:"rgba(201,168,76,.05)",border:`1px solid ${match?scoreColor+"55":"rgba(201,168,76,.18)"}`,borderRadius:10,padding:"6px 4px",alignSelf:"flex-start",cursor:"pointer"}}>
...
function FSection({title,count,children,onClear}) {
  ...
  return <div style={{border:"1px solid rgba(201,168,76,.1)",borderRadius:10,overflow:"hidden",marginBottom:4,flexShrink:0}}>
    <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",background:"rgba(201,168,76,.05)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:8,padding:"9px 12px",fontFamily:"'Cinzel',serif",color:"rgba(244,237,216,.6)",fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:.8,transition:"background .15s"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(201,168,76,.09)"} onMouseLeave={e=>e.currentTarget.style.background="rgba(201,168,76,.05)"}>
      ...
      {count>0&&onClear&&<span onClick={e=>{e.stopPropagation();onClear();}} title="Clear this category" style={{fontSize:11,color:"rgba(232,97,58,.7)",cursor:"pointer",padding:"0 2px",lineHeight:1}}>✕</span>}
    </button>
...
// ── MAIN APP ──────────────────────────────────────────────────────────────────
// US state polygons (Natural Earth, public domain), simplified for the Journey map.
...
const CA_PROVINCES_GEO={"AB":{"name":"Alberta","rings":[[[-110.0,48.99],[-114.06,48.99],[-114.73,49.59],[-114.99,50.55],[-118.22,52.39],[-118.8,53.15],[-119.89,53.52],[-120,60.0],[-110.0,60.0],[-110.0,48.99]]],"c":[-115.07,52.62]},"BC":{"name":"British Columbia","rings":[[[-120,60.0],[-119.89,53.52],[-118.8,53.15],[-118.22,52.39],[-114.99,50.55],[-114.73,49.59],[-114.06,48.99],[-123.09,48.99],[-122.88,49.4],[-123.28,49.34],[-123.19,49.68],[-123.53,49.4],[-124.03,49.6],[-123.58,49.68],[-123.83,50.14],[-124.28,49.77],[-124.78,50.02],[-124.86,50.87],[-125.06,50.51],[-126.09,50.5],[-126.45,50.59],[-125.9,50.7],[-126.51,50.68],[-126.52,51.06],[-127.71,51.15],[-127.42,51.61],[-126.69,51.7],[-127.85,51.67],[-127.44,52.36],[-126.71,52.06],[-127.19,52.46],[-127.01,52.84],[-128.1,51.79],[-128.36,52.16],[-128.05,52.91],[-129.17,53.53],[-127.93,53.27],[-128.68,53.55],[-128.53,53.86],[-129.56,53.25],[-130.34,53.72],[-129.63,54.23],[-130.43,54.42],[-129.56,55.46],[-130.09,55.11],[-130.06,56.07],[-131.82,56.59],[-133.4,58.41],[-135.05,59.58],
-70.65,60.03],[-69.73,59.92],[-69.68,59.34],[-69.34,59.3],[-69.53,58.87],[-70.15,58.76],[-68.38,58.74],[-68.36,58.16],[-69.04,57.9],[-68.02,58.49],[-67.68,57.99],[-66.36,58.79],[-66.0,58.43],[-66.04,58.82],[-65.38,59.06],[-65.7,59.21],[-65.48,59.47],[-65.04,59.39],[-65.43,59.78],[-64.61,60.31]],[[-61.8,49.09],[-64.49,49.89],[-62.86,49.71],[-61.8,49.09]]],"c":[-69.68,55.6]},"SK":{"name":"Saskatchewan","rings":[[[-110.0,48.99],[-110.0,60.0],[-102.0,60.0],[-101.37,48.99],[-110.0,48.99]]],"c":[-106.67,53.39]},"YT":{"name":"Yukon","rings":[[[-123.82,60.0],[-139.06,60.0],[-139.08,60.34],[-141.0,60.3],[-141.0,69.65],[-136.44,68.9],[-136.17,67.02],[-134.06,67.0],[-133.62,65.97],[-132.34,65.98],[-132.19,65.6],[-132.77,65.2],[-132.35,65.06],[-132.59,64.79],[-130.12,63.79],[-129.84,63.49],[-130.13,63.28],[-129.61,63.06],[-129.25,62.15],[-127.17,61.46],[-126.86,60.77],[-124.58,60.95],[-123.82,60.0]]],"c":[-132.08,63.69]}};
const STATE_NAME_TO_POSTAL=(()=>{const m={};for(const[pc,s]of Object.entries(US_STATES_GEO))m[s.name.toLowerCase()]=pc;for(const[pc,s]of Object.entries(CA_PROVINCES_GEO))m[s.name.toLowerCase()]=pc;return m;})();
...
function AiUsageBar(){
  ...
  const load=async()=>{ try{ const {data:s}=await supabase.auth.getSession(); const tk=s&&s.session&&s.session.access_token; if(!tk)return; const r=await fetch("/api/ai/usage",{headers:{Authorization:`Bearer ${tk}`}}); const j=await r.json().catch(()=>({})); if(r.ok)setData(j); }catch(e){} };
  useEffect(()=>{ load(); const onVis=()=>{ if(!document.hidden) load(); }; document.addEventListener("visibilitychange",onVis); return ()=>document.removeEventListener("visibilitychange",onVis); },[]);
  useEffect(()=>{ if(!open)return; load(); const t=setInterval(()=>setNow(n=>n+1),1000); return ()=>clearInterval(t); },[open]);
  if(!data) return null;
  const isAdmin=data.isAdmin;
  const limit=data.limit||40;
  const remaining=isAdmin?limit:Math.max(0,limit-(data.used||0));
  const pct=isAdmin?100:Math.round((remaining/limit)*100);
  const barColor=isAdmin?"linear-gradient(90deg,#7ecfb3,#c9a84c)":pct>50?"linear-gradient(90deg,#c9a84c,#f0d080)":pct>20?"#e8a070":"#c0703a";
  const cd=()=>{ const d=new Date(); const next=new Date(d.getFullYear(),d.getMonth()+1,1,0,0,0,0); let ms=next-d; const days=Math.floor(ms/86400000); ms-=days*86400000; const hrs=Math.floor(ms/3600000); ms-=hrs*3600000; const mins=Math.floor(ms/60000); ms-=mins*60000; const secs=Math.floor(ms/1000); return `${days}d ${hrs}h ${mins}m ${secs}s`; };
  return <div style={{position:"relative",flexShrink:0}} onMouseEnter={()=>setOpen(true)} onMouseLeave={()=>setOpen(false)}>
    <button onClick={()=>setOpen(o=>!o)} title="AI uses this month" style={{display:"flex",alignItems:"center",gap:8,background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.18)",borderRadius:20,padding:"6px 12px",cursor:"pointer"}}>
      <I.Lightning s={13} c="#c9a84c"/>
      <div style={{width:60,height:6,background:"rgba(201,168,76,.14)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:barColor,borderRadius:4,transition:"width .45s ease"}}/></div>
      <span style={{fontSize:11,fontWeight:700,color:"#f0d080",fontFamily:"'Cinzel',serif",whiteSpace:"nowrap"}}>{isAdmin?"\u221e":`${remaining}/${limit}`}</span>
    </button>
    {open&&<div style={{position:"absolute",top:44,right:0,width:216,background:"#100a16",border:"1px solid rgba(201,168,76,.25)",borderRadius:10,padding:12,zIndex:300,boxShadow:"0 8px 30px rgba(0,0,0,.5)"}}>
      <div style={{fontFamily:"'Cinzel',serif",fontSize:11,fontWeight:800,color:"#f0d080",marginBottom:6,letterSpacing:.5}}>AI Uses</div>
      <div style={{fontSize:12,color:"rgba(244,237,216,.72)",marginBottom:7}}>{isAdmin?"Unlimited (admin)":`${remaining} of ${limit} left this month`}</div>
      <div style={{fontSize:11,color:"rgba(244,237,216,.5)"}}>Resets in <span style={{color:"#c9a84c",fontWeight:700,fontFamily:"'Cinzel',serif"}}>{cd()}</span></div>
      <div style={{fontSize:10,color:"rgba(244,237,216,.35)",marginTop:6,lineHeight:1.4}}>Shared across resume tailoring, company info, interview prep &amp; templates.</div>
    </div>}
  </div>;
}
function GeneratedResumes({ user, onPanel }){
  ...
  return <div style={{textAlign:"left"}}>
    ...
    {sel&&typeof document!=="undefined"&&createPortal(<div style={mobile?{position:"fixed",inset:0,zIndex:500,background:"rgba(10,7,10,.98)",padding:"calc(env(safe-area-inset-top, 0px) + 16px) 16px calc(env(safe-area-inset-bottom, 0px) + 90px)",overflowY:"auto",WebkitOverflowScrolling:"touch"}:{position:"fixed",top:96,right:16,width:"50vw",maxHeight:"calc(100vh - 112px)",overflowY:"auto",zIndex:60,background:"rgba(16,10,22,.97)",backdropFilter:"blur(24px)",border:"1px solid rgba(201,168,76,.3)",borderRadius:14,padding:18,boxShadow:"0 24px 70px rgba(0,0,0,.7)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:12,position:mobile?"sticky":"static",top:0,zIndex:5,background:mobile?"rgba(10,7,10,.98)":"transparent",marginLeft:mobile?-16:0,marginRight:mobile?-16:0,paddingLeft:mobile?16:0,paddingRight:mobile?16:0}}>
...
function InterviewPrepPanel({ job, onClose }){
  ...
  return <div style={mobile?{position:"fixed",inset:0,zIndex:400,background:"rgba(10,7,10,.98)",padding:"calc(env(safe-area-inset-top, 0px) + 16px) 16px calc(env(safe-area-inset-bottom, 0px) + 90px)",overflowY:"auto",WebkitOverflowScrolling:"touch"}:{position:"fixed",top:96,right:16,width:"50vw",maxHeight:"calc(100vh - 112px)",overflowY:"auto",zIndex:60,background:"rgba(16,10,22,.97)",backdropFilter:"blur(24px)",border:"1px solid rgba(201,168,76,.3)",borderRadius:14,padding:18,boxShadow:"0 24px 70px rgba(0,0,0,.7)"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:12,borderBottom:"1px solid rgba(201,168,76,.15)",paddingBottom:10,position:mobile?"sticky":"static",top:0,zIndex:5,background:mobile?"rgba(10,7,10,.98)":"transparent",marginLeft:mobile?-16:0,marginRight:mobile?-16:0,paddingLeft:mobile?16:0,paddingRight:mobile?16:0}}>
...
export default function App() {
  ...
  const fetchLiveJobs=async(isIntro=false)=>{
    ...
    const fetchGroup=async(members)=>{
      ...
      // Per-request timeout so a single slow/hung ATS endpoint can't stall the
      // whole load. Still respects the global abort (navigation/unmount).
  ...
  useEffect(()=>{
    if((user||guest)&&!introStartedRef.current){
      ...
      // to finish (each request is time-boxed to 9s), so the board only reveals
      // once fully populated. This cap only fires in a pathological case (a very
      // cold cache with many slow endpoints) so the user is never trapped forever.
  ...
  const [filterOpen,setFilterOpen]=useState(() => typeof window !== "undefined" ? window.innerWidth > 1780 : false);
  ...
  // more than one state used to have their jobs counted once per listing, which
  // inflated this number. Passing no region means no location filtering, so jobs in
  // every region (including "Other Locations") are included.
  const allJobs=useMemo(()=>{
    const seen=new Set();
    const out=[];
    for(const states of Object.values(ALL_JOBS_DATA))
      for(const companies of Object.values(states))
        for(const [nm,co] of Object.entries(companies)){
          if(seen.has(nm)) continue;
          seen.add(nm);
          out.push(...getDisplayJobs(nm,co.jobs));
        }
    return out;
  },[liveJobs]);
  ...
  const journeyDots=useMemo(()=>{
    const dots=[];
    for(const [country,states] of Object.entries(ALL_JOBS_DATA)){
      const geoSet=country==="Canada"?CA_PROVINCES_GEO:US_STATES_GEO;
      for(const [stateName,companies] of Object.entries(states)){
        const pc=STATE_NAME_TO_POSTAL[(stateName||"").toLowerCase()];
        const geo=pc&&geoSet[pc];
        if(!geo) continue; // skip states/provinces we have no polygon for
  ...
  // Autocomplete option lists for the job-alert wizard.
  const companyOptions=useMemo(()=>[...new Set(allJobs.map(j=>j.company).filter(Boolean))].sort(),[allJobs]);
  const locationOptions=useMemo(()=>{
    const s=new Set(["Remote"]);
    for(const c of allCountries) s.add(c);
    for(const st of allStates) s.add(st);
    for(const k of Object.keys(CITY_COUNTRY)) s.add(titleCasePlace(k));
  ...
  return <>
    ...
    <main style={{position:"relative",zIndex:1,maxWidth:1100,width:"100%",margin:(breakdownJob&&!mobile&&(tab==="jobs"||tab==="saved"))?"0 calc(min(46vw,720px) + 40px) 0 24px":(resumePanel&&!mobile&&tab==="saved")?"0 calc(50vw + 24px) 0 24px":(prepJob&&!mobile&&tab==="applied")?"0 calc(50vw + 24px) 0 24px":"0 auto",transition:"margin .35s ease",padding:mobile?"14px 12px":"24px 18px",flex:1}}>
      {tab==="jobs"&&<>
        ...
        <div style={{display:"flex",flexDirection:(filterInline||(breakdownJob&&tab==="jobs"))?"column":"row",gap:16,alignItems:"flex-start"}}>
          {filterOpen&&<div style={(filterInline||(breakdownJob&&tab==="jobs"))?{order:0,position:"relative",width:"100%",flexShrink:0,background:"rgba(16,10,22,.9)",backdropFilter:"blur(20px)",border:"1px solid rgba(201,168,76,.18)",borderRadius:14,padding:"34px 12px 12px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(215px,1fr))",gap:"6px 14px",alignItems:"start",marginBottom:4}:{position:"fixed",top:96,left:"max(calc(50% - 878px), 12px)",width:312,maxHeight:"calc(100vh - 112px)",overflowY:"auto",overscrollBehavior:"contain",zIndex:60,background:"rgba(16,10,22,.97)",backdropFilter:"blur(24px)",border:"1px solid rgba(201,168,76,.3)",borderRadius:14,padding:"36px 16px 16px",display:"flex",flexDirection:"column",gap:4,boxShadow:"0 24px 70px rgba(0,0,0,.7)"}}><button onClick={()=>setFilterOpen(false)} title="Close filters" style={{position:"absolute",top:8,right:10,zIndex:3,width:26,height:26,borderRadius:8,background:"rgba(201,168,76,.1)",border:"1px solid rgba(201,168,76,.25)",color:"#c9a84c",cursor:"pointer",fontSize:13,lineHeight:1,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          ...
          <div style={{order:1,flex:1,minWidth:0,width:(filterInline||(breakdownJob&&tab==="jobs"))?"100%":"auto",display:"flex",flexDirection:"column"}}>
        {/* Sort bar */}
                                  ...
                                  {noJobs
                                    ...
                                    :(()=>{
                                      ...
                                      return keys.map(k=>{
                                        ...
                                        const locOpen=expanded[locKey]!==false; // default open
                                        return <div key={k} style={{display:"flex",flexDirection:"column",gap:5}}>
                                          <div onClick={()=>toggle(locKey)} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 4px 3px",marginTop:2,cursor:"pointer",borderRadius:6}} onMouseEnter={e=>e.currentTarget.style.background="rgba(201,168,76,.05)"} onMouseLeave={e=>e.currentTarget.style.background=""}>
                                            <span style={{display:"inline-flex",color:"rgba(201,168,76,.5)"}}><I.Chevron s={10} c="currentColor" dir={locOpen?"down":"right"}/></span>
                                            ...
                                            <span style={{fontSize:8.5,color:"rgba(244,237,216,.3)"}}>{groups[k].length} {groups[k].length===1?"job":"jobs"}</span>
                                            <span style={{flex:1,height:1,background:"rgba(201,168,76,.08)"}}/>
                                          </div>
                                          {locOpen&&groups[k].map(renderJob)}
                                        </div>;
                                      });
                                    })()}
                                </div>}
      ...
      {tab==="applied"&&<div>
        ...
        {appliedJobs.length===0?<div style={{padding:"48px 0",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
          ...
          {[...appliedJobs].filter(j=>STAGE_OF(user.applied[j.id]&&user.applied[j.id].status)===appStage).sort((a,b)=>{
            ...
            return <div key={job.id} style={{background:"rgba(16,10,22,.6)",border:"1px solid rgba(201,168,76,.14)",borderRadius:12,padding:"14px 16px",display:"flex",flexDirection:"column",gap:12,position:"relative"}}><button onClick={()=>removeApplied(job.id)} title="Remove" style={{position:"absolute",top:12,right:12,zIndex:2,background:"rgba(192,50,26,.08)",border:"1px solid rgba(192,50,26,.2)",color:"rgba(232,120,90,.7)",cursor:"pointer",width:26,height:26,borderRadius:8,fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",transition:"all .15s"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(192,50,26,.2)";e.currentTarget.style.color="#e87060";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(192,50,26,.08)";e.currentTarget.style.color="rgba(232,120,90,.7)";}}><I.X s={11} c="currentColor"/></button><div style={{paddingRight:34}}><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:6}}><span style={{background:`${si.color}22`,border:`1px solid ${si.color}66`,color:si.color,borderRadius:20,fontSize:10,padding:"2px 10px",fontWeight:700,fontFamily:"'Cinzel',serif"}}>{si.label}</span><span style={{fontSize:12,fontWeight:600,color:"rgba(244,237,216,.6)",fontFamily:"'Cinzel',serif"}}>{job.company}</span><span style={{fontSize:10,color:"rgba(244,237,216,.4)",background:"rgba(201,168,76,.06)",padding:"2px 9px",borderRadius:20}}>{job.state}</span></div><div style={{fontFamily:"'Cinzel',serif",fontSize:16,fontWeight:700,color:"#f4edd8",marginBottom:6,letterSpacing:.5}}>{job.title}</div><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{[job.type,job.isRemote&&"Remote OK",(job.salary&&job.salary!=="Salary not listed")&&job.salary,`Posted ${job.postedStr}`].filter(Boolean).map((c,i)=><span key={i} style={{background:"rgba(201,168,76,.07)",border:"1px solid rgba(201,168,76,.15)",borderRadius:20,fontSize:10,padding:"2px 9px",color:"rgba(244,237,216,.6)"}}>{c}</span>)}</div></div><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",gap:12,flexWrap:"wrap"}}><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><a href={job.applyUrl||job.url} target="_blank" rel="noreferrer" style={{textDecoration:"none",display:"inline-flex",alignItems:"center",gap:5,background:"rgba(201,168,76,.12)",border:"1px solid rgba(201,168,76,.25)",color:"#f0d080",borderRadius:8,padding:"6px 14px",fontSize:11,fontFamily:"'Cinzel',serif",fontWeight:700,letterSpacing:.3}}>View Posting <I.Arrow s={11} c="#f0d080"/></a><InterviewPrepButton job={job} onOpen={setPrepJob}/></div><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",justifyContent:"flex-end"}}>{STAGE_ACTIONS[STAGE_OF(st)].length>0?<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{STAGE_ACTIONS[STAGE_OF(st)].map(([v,l,c])=><button key={v} onClick={()=>setAppStatus(job.id,v)} style={{background:`${c}18`,border:`1px solid ${c}55`,color:c,borderRadius:8,padding:"6px 12px",fontSize:10.5,fontFamily:"'Cinzel',serif",fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>{l}</button>)}</div>:<button onClick={()=>setAppStatus(job.id,"applied")} title="Reset to Applied" style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",color:"rgba(244,237,216,.45)",borderRadius:8,padding:"6px 11px",fontSize:10,fontFamily:"'Cinzel',serif",cursor:"pointer",whiteSpace:"nowrap"}}>↺ Reset</button>}<div style={{background:"rgba(126,207,179,.07)",border:"1px solid rgba(126,207,179,.2)",borderRadius:10,padding:"6px 12px",textAlign:"center"}}><div style={{fontSize:9,color:"#7ecfb3",textTransform:"uppercase",letterSpacing:.8,fontFamily:"'Cinzel',serif",marginBottom:2}}>Applied</div><div style={{fontSize:11,color:"#f4edd8",fontWeight:600,whiteSpace:"nowrap"}}>{appliedDate}</div></div></div></div></div>;
          })}
        </div>}
      </div>}
    ...
    {/* Legal footer */}
    <footer style={{borderTop:"1px solid rgba(201,168,76,.12)",padding:"20px 24px",marginTop:0,marginRight:(!mobile&&((prepJob&&tab==="applied")||(resumePanel&&tab==="saved")))?"50vw":(breakdownJob&&!mobile&&(tab==="jobs"||tab==="saved"))?"min(46vw,720px)":0,transition:"margin .35s ease",display:"flex",flexWrap:"wrap",alignItems:"center",justifyContent:"space-between",gap:12,background:"rgba(8,6,8,.6)",position:"relative",zIndex:1,flexShrink:0}}>
      <div style={{fontSize:11,color:"rgba(244,237,216,.35)",lineHeight:1.5,maxWidth:560}}>
        Main Quest aggregates publicly available job listings and is not affiliated with any studio listed. Job data may be inaccurate — always verify on the employer's official site. Trademarks belong to their respective owners.
      </div>
      <div style={{display:"flex",gap:16,alignItems:"center",flexShrink:0}}>
