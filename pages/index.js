...
const JobCard = memo(function JobCard({job,user,guest,onRequestLogin,onApplied,onShowBreakdown,isPremium,onToggleSave,activeBreakdown,flatView,notifyOn,onToggleNotify}) {
  ...
  return <div style={{perspective:"1400px"}}><div style={{position:"relative",transformStyle:"preserve-3d",transition:"transform .55s ease",transform:flipped?"rotateY(180deg)":"none"}}><div style={{backfaceVisibility:"hidden",WebkitBackfaceVisibility:"hidden",position:flipped?"absolute":"relative",top:0,left:0,width:"100%"}}><div style={{background:"rgba(16,10,22,.6)",border:`1px solid ${isApplied?"rgba(126,207,179,.3)":job.isNew?"rgba(192,50,26,.35)":"rgba(201,168,76,.12)"}`,borderRadius:10,padding:"13px 15px",transition:"all .2s",cursor:"default",position:"relative",boxShadow:activeBreakdown?"0 0 0 2px rgba(201,168,76,.6), 0 6px 26px rgba(201,168,76,.14)":"none"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(201,168,76,.05)";e.currentTarget.style.transform="translateX(3px)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(16,10,22,.6)";e.currentTarget.style.transform="";}}>
    ...
    <div style={{display:"flex",gap:8,flexWrap:mobile?"nowrap":"wrap",alignItems:"center"}}>
      ...
      {(cContact||cEmail)&&<div style={mobile?{display:"flex",gap:6,flexShrink:0,flexWrap:"wrap",justifyContent:"flex-end",marginLeft:"auto"}:{position:"absolute",bottom:13,right:15,zIndex:4,display:"flex",gap:6,flexWrap:"wrap",justifyContent:"flex-end"}}>
    ...
    {user&&!_isOpenAppJob(job)&&<div onClick={e=>{e.stopPropagation(); if(!match){setShowScoreInfo(v=>!v);} else if(smallBreak){setFlipped(true);} else if(onShowBreakdown){onShowBreakdown(job);}}} title="Click for the full match breakdown" style={{flexShrink:0,width:mobile?58:66,position:"relative",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,background:match?`${scoreColor}1a`:"rgba(201,168,76,.05)",border:`1px solid ${match?scoreColor+"55":"rgba(201,168,76,.18)"}`,borderRadius:10,padding:"6px 4px",alignSelf:"flex-start",cursor:"pointer"}}>
